...
def show_toast(msg, duration=2000):
    ...
    toast.configure(fg_color="#222222")
    
    label = ctk.CTkLabel(toast, text=msg, text_color="white")
    label.pack(padx=20, pady=10)
    
    # Position bottom-right
    app.update_idletasks()
    x = app.winfo_x() + app.winfo_width() - toast.winfo_reqwidth() - 30
    y = app.winfo_y() + app.winfo_height() - toast.winfo_reqheight() - 60
...
def _do_calibration():
    ...
    # 3) Schedule the UI updates back on the main thread
    app.after(0, lambda: home_box_add(f"🔧 Mic calibrated: threshold = {int(level)}"))
    app.after(0, restart_listener)

def calibrate_mic():
    show_toast("🎙️ Calibrating microphone… please be quiet", duration=2000)
    threading.Thread(target=_do_calibration, daemon=True).start()


calibrate_button = ctk.CTkButton(
    inner,
    text="Calibrate for background audio",
    width=80,
    height=30,
    command=calibrate_mic
)
calibrate_button.grid(row=6, column=1, sticky="w", padx=250, pady=5)
...
def save_settings():
    ...
    try:
        ...
        show_toast("✅ Settings successfully saved.", duration=2000)
    except Exception as e:
...
def restart_listener():
    ...
    app.after(0, lambda: show_toast(f"🎙️ Calibration complete \n ambient noise level {int(ret)}"
    , duration=1800))
...
# note: add a reverse talking mode, where lack of talking gets the user
# shocked. they get to set the time intervals
